...
//        assertEquals(-1, parents[2][2]);
   ...
   @Test
    ...
    @Test
    void Test_Costs_For_Negative_Edges_Graph_100_Vertices_656_edges() {
        Graph graph = new Graph("Input/graph7.txt");
        int[][] floydCosts = new int[graph.size()][graph.size()];
        int[][] parents = new int[graph.size()][graph.size()];
        graph.floydWarshall(floydCosts, parents);
        int[][] BellmanCosts = new int[graph.size()][graph.size()];
        for(int i=0;i<graph.size();i++)
            graph.BellmanFord(i,BellmanCosts[i], parents[i]);

        for(int i=0;i<graph.size();i++)
            for(int j=0;j< graph.size();j++)
                assertEquals(floydCosts[i][j], BellmanCosts[i][j]);
    }

    @Test
    void Test_Costs_For_Negative_Edges_Graph_500_Vertices_7000_edges() {
        Graph graph = new Graph("Input/graph39.txt");
        int[][] floydCosts = new int[graph.size()][graph.size()];
        int[][] parents = new int[graph.size()][graph.size()];
        graph.floydWarshall(floydCosts, parents);
        int[][] BellmanCosts = new int[graph.size()][graph.size()];
    ...
    @Test
    void Test_Costs_For_Positive_Edges_Graph_500_Vertices_8000_edges() {
        Graph graph = new Graph("Input/graph40.txt");
        int[][] DijkstraCosts = new int[graph.size()][graph.size()];
        int[][] DijkstraParents = new int[graph.size()][graph.size()];
        for(int i=0;i< graph.size();i++)
            graph.Dijkstra(i,DijkstraCosts[i],DijkstraParents[i]);

        int[][] BellmanCosts = new int[graph.size()][graph.size()];
        int[][] BellmanParents = new int[graph.size()][graph.size()];
        for(int i=0;i< graph.size();i++)
            graph.BellmanFord(i,BellmanCosts[i],BellmanParents[i]);

        int[][] floydCosts = new int[graph.size()][graph.size()];
        int[][] floydParents = new int[graph.size()][graph.size()];
        graph.floydWarshall(floydCosts,floydParents);

        for(int i=0;i< graph.size();i++){
            for(int j=0;j< graph.size();j++){
                assertEquals(BellmanCosts[i][j], DijkstraCosts[i][j]);
                assertEquals(floydCosts[i][j], DijkstraCosts[i][j]);
                assertEquals(BellmanCosts[i][j], floydCosts[i][j]);
            }
        }
    }

    @Test
    void Test_Costs_For_Positive_Edges_Graph_250_Vertices_20000_edges() {
        Graph graph = new Graph("Input/graph41.txt");
        int[][] DijkstraCosts = new int[graph.size()][graph.size()];
        int[][] DijkstraParents = new int[graph.size()][graph.size()];
        for(int i=0;i< graph.size();i++)
            graph.Dijkstra(i,DijkstraCosts[i],DijkstraParents[i]);
    ...
    @Test
    void NegativeCycles_Edges_Graph_46_Vertices_856_edges() {
        Graph graph = new Graph("Input/graph8.txt");
        int[][] costs = new int[graph.size()][graph.size()];
        int[][] parents = new int[graph.size()][graph.size()];
        assertFalse(graph.floydWarshall(costs, parents));
        assertFalse(graph.BellmanFord(0,costs[0], parents[0]));
    }
